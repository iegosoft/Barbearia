...
export default function Admin() {
  ...
  const carregarAgendamentos = useCallback(async (f = filtros) => {
    try {
      setCarregando(true)
  ...
  async function atualizarStatus(id, status) {
    ...
    try {
      ...
      setAgendamentos(prev => prev.map(a => a._id === id ? { ...a, status } : a))
    } finally {
  ...
  function limparFiltros() {
    ...
    setFiltros(limpo)
    carregarAgendamentos(limpo)
  }

  // Stats calculadas localmente
  const hoje = new Date().toISOString().split('T')[0]
  const statsHoje     = agendamentos.filter(a => a.data === hoje)
  const pendentesHoje = statsHoje.filter(a => a.status === 'pendente').length
  const concluidosHoje = statsHoje.filter(a => a.status === 'concluido').length
  const totalHoje     = statsHoje.length
  const totalPendentes = agendamentos.filter(a => a.status === 'pendente').length

  const temFiltro = filtros.data || filtros.barbeiro || filtros.status
      ...
      <div className="flex min-h-[calc(100vh-3.5rem)]">
        ...
        <main className="flex-1 p-6 lg:p-8 min-w-0">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              { label: 'Hoje', valor: totalHoje,       cor: 'text-white'         },
              { label: 'Pendentes',  valor: totalPendentes, cor: 'text-amber-400'  },
              { label: 'Concluídos hoje', valor: concluidosHoje, cor: 'text-emerald-400' },
              { label: 'Agend. hoje c/ pendência', valor: pendentesHoje, cor: 'text-blue-400' },
            ].map(s => (
                ...
                {agendamentos.map(a => (
                  <div key={a._id} className="bg-dark-900 rounded-xl border border-dark-700 p-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {a.status !== 'concluido' && (
                        <button onClick={() => atualizarStatus(a._id, 'concluido')} className="flex-1 text-xs border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 py-2 rounded transition-all cursor-pointer">✓ Concluído</button>
                      )}
                      {a.status !== 'remarcado' && (
                        <button onClick={() => atualizarStatus(a._id, 'remarcado')} className="flex-1 text-xs border border-blue-400/30 text-blue-400 hover:bg-blue-500/10 py-2 rounded transition-all cursor-pointer">↩ Remarcar</button>
                      )}
                      {a.status !== 'cancelado' && (
                        <button onClick={() => atualizarStatus(a._id, 'cancelado')} className="flex-1 text-xs border border-red-500/30 text-red-400 hover:bg-red-500/10 py-2 rounded transition-all cursor-pointer">✗ Cancelar</button>
                      )}
